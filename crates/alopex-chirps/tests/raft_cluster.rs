use alopex_chirps::{ChirpsRaftTransport, RaftConfig, RaftNode, RaftError};
...
use openraft::{
    CommittedLeaderId, ErrorSubject, ErrorVerb, OptionalSend, RaftLogReader, RaftStorage as OpenRaftStorage,
...
impl OpenRaftStorage<ChirpsTypeConfig> for MemoryStore {
    ...
    async fn save_vote(&mut self, vote: &Vote<ChirpsNodeId>) -> Result<(), StorageError<ChirpsNodeId>> {
        let mut guard = self.inner.lock().await;
        guard.vote = Some(vote.clone());
        Ok(())
    }

    async fn read_vote(&mut self) -> Result<Option<Vote<ChirpsNodeId>>, StorageError<ChirpsNodeId>> {
        let guard = self.inner.lock().await;
        Ok(guard.vote.clone())
    }

    async fn get_log_state(&mut self) -> Result<LogState<ChirpsTypeConfig>, StorageError<ChirpsNodeId>> {
        let guard = self.inner.lock().await;
        Ok(LogState {
    ...
    async fn read_committed(&mut self) -> Result<Option<LogId<ChirpsNodeId>>, StorageError<ChirpsNodeId>> {
        let guard = self.inner.lock().await;
        ...
        for entry in entries {
            ...
            match &entry.payload {
                ...
                EntryPayload::Membership(m) => {
                    guard.last_membership = StoredMembership::new(Some(entry.log_id.clone()), m.clone());
    ...
    async fn install_snapshot(
        ...
        let restored: Vec<Vec<u8>> = if buf.is_empty() {
            ...
            bincode::deserialize(&buf).map_err(|e| {
                to_io_error(ErrorSubject::Snapshot(None), ErrorVerb::Read, e)
            })?
        };
...
impl TestCluster {
    ...
    async fn wait_for_state_len(
        ...
        loop {
            ...
            if start.elapsed() > timeout {
                bail!("state length did not reach {} within {:?}", expected, timeout);
            }
...
fn membership_voters(metrics: &openraft::metrics::RaftMetrics<ChirpsNodeId, BasicNode>) -> BTreeSet<ChirpsNodeId> {
    metrics
        .membership_config
        .membership()
        .get_joint_config()
